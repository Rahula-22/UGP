...
export function getBotReply(raw: string): string {
  ...
  if (/(schedule|itinerary|timeline|\bevents?\b|program|when.*(start|begin)|\bdates?\b)/.test(q))
    return scheduleText();

  if (/(wear|dress|outfit|attire|clothes|saree|\bsari\b|lehenga|sherwani|kurta)/.test(q))
    return dressText();

  if (/(venue|where|location|address|\bplace|reach|directions?|\bmap\b)/.test(q))
    return venueText();

  if (/(food|menu|\beat\b|dinner|lunch|\bveg|jain|gluten|dietary|allerg)/.test(q))
  ...
  if (/(hotel|\bstay\b|\broom|accommodat|lodging|check.?in)/.test(q))
  ...
  if (/(\bpark|\bcars?\b|taxi|\bcabs?\b|shuttle|transport|travel|airport|train|flight)/.test(q))
  ...
  if (/(photo|camera|instagram|hashtag|social media)/.test(q))
  ...
  if (/(weather|\bhot\b|\bcold\b|\brain|temperature|climate)/.test(q))
  ...
  if (/(\bkids?\b|children|\bbaby\b|family friendly)/.test(q))
  ...
  if (/(contact|planner|\bhelp\b|phone|\bnumber\b|organi[sz]er|emergency)/.test(q))
  ...
  if (/(thank|shukriya|dhanyavad|\bgreat\b|awesome|\bnice\b)/.test(q))
